...
const Navbar = () => {
  ...
  return (
    ...
    >
      ...
      <button onClick={handleToggleMenu} className="sm:hidden" aria-label="Menu">
      ...
      >
        ...
        <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
          <button onClick={handleListCarsOrDashboard} className="cursor-pointer">
            {isOwner ? 'Dashboard' : 'List Cars'}
          </button>

          <button
            onClick={handleLoginOrLogout}
            className="cursor-pointer px-8 bg-primary hover:bg-primary-dull transition-all text-white rounded-lg"
          >
            {user ? 'Logout' : 'Login'}
          </button>
        </div>
